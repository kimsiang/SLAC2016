...
void TimeStudyAna::execute(){
    ...
    for(size_t iF =0;iF<XtalHit_EventNum->size();iF++){  
        ...
        if(XtalHit_Time->at(iF) < 1000) {
            //cout<<"Sync XtalHit["<<XtalHit_XtalNum->at(iF)<< "], Time: "<<XtalHit_Time->at(iF)<<", Energy: "<<XtalHit_Energy->at(iF)<<endl;
    ...
    for(size_t iC =0;iC<Cluster_EventNum->size();iC++){  
        ...
        if(Cluster_Energy->at(iC) < 3000 && Cluster_Energy->at(iC) > 1000 && Cluster_Time->at(iC)<2000 ) {
            isSingleElec = true; 
            islandNum = Cluster_IslandNum->at(iC);
    ...
    if(isSingleElec) {
        ...
        for(size_t iF =0;iF<XtalHit_EventNum->size();iF++){  
            ...
            if(XtalHit_Time->at(iF) > 1000 && XtalHit_Time->at(iF)<2000) {
                //cout<<"Beam XtalHit["<<XtalHit_XtalNum->at(iF)<< "], Time: "<<XtalHit_Time->at(iF)<<", Energy: "<<XtalHit_Energy->at(iF)<<endl;
