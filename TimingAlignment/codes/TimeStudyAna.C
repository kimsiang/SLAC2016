...
void TimeStudyAna::finalize(string &filename){
    ...
    for(int i=0;i<4;i++){
        ...
        if(fit->GetParameter(2)<0.1) continue;
        ...
        syncTimeDiff[channel_i[i]][channel_j]->Fit("fitd","QMER");
        cout<<"mean["<< channel_i[i]<<"]["<< channel_j<<"]: "<< fitd->GetParameter(1)<<", rms: "<<fitd->GetParameter(2)<<endl;
        delete fitd; 
